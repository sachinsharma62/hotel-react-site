...
const Roomsdata = [
  {
    ...
    star: <><FaStar /> <FaStar /> <FaStar /> <FaStarHalf /></>,
    bed: '2 Bed',
    bath: '2 Bath',
    Wifi: 'Wifi',
  ...
  },
  {
    ...
    roomrate: '$100/Night',
    textheading: 'Super Deluxe',
    star: <><FaStar /> <FaStar /> <FaStar /> <FaStar /> <FaStarHalf /></>,
    bed: '3 Bed',
    bath: '2 Bath',
    Wifi: 'Wifi',
    textdetails: 'Erat ipsum justo amet duo et elitr dolor, est duo duo eos lorem sed diam stet diam sed stet lorem.'
          ...
          <Col key={item.key} md={6} lg={4} className='mt-4'>
            <div className='room-item shadow rounded'>
              <div className='position-relative'>
                <img className='img-fluid' src={item.roomimg} alt={item.textheading} />
                <small className='position-absolute start-0 top-100 translate-middle-y back-primary text-white py-1 px-3 ms-4'>{item.roomrate}</small>
              </div>
              <div className='p-4 mt-2'>
                <div className='d-flex justify-content-between mb-3'>
                  <h5 className='textDark font-weight-bold'>{item.textheading}</h5>
                  <div className='textPrimary ps-2'>{item.star}</div>
                </div>
                <div className='d-flex mb-3'>
                  <small className='border-end me-3 pe-3 d-flex align-items-center'>
                    <FaBed className='textPrimary me-3 f-19' />{item.bed}
                  </small>
                  <small className='border-end me-3 pe-3 d-flex align-items-center'>
                    <FaBath className='textPrimary me-3 f-19' />{item.bath}
                  </small>
                  <small className='me-3 pe-3 d-flex align-items-center'>
                    <FaWifi className='textPrimary me-3 f-19' />{item.Wifi}
                  </small>
                </div>
                <p className="text-body mb-3 f-19">{item.textdetails}</p>
                <div className='d-flex justify-content-between'>
                  <Button type='button' className='btn-primary rounded-0 py-2 px-4'>View Detail</Button>
                  <Button type='button' className='btn-dark rounded-0 py-2 px-4'   onClick={() => handleBooking(item)}>Book Now</Button>
                </div>
              </div>
            </div>
          </Col>
